...
2. Pick one file from `queue/` (author **new** tasks only in `queue/`, not under `runs/`). Name the file **`YYYY-MM-DD-short-slug.md`** — do **not** use a generic `prompt.md` (hard to track and duplicates archive text).
